...
export default function App() {
  ...
  const [appliedCoupon, setAppliedCoupon] = useState(() => {
    try {
      const saved = localStorage.getItem('gga_coupon');
      return saved ? JSON.parse(saved) : null;
    } catch (e) {
      return null;
    }
  ...
  useEffect(() => {
    if (appliedCoupon) {
      localStorage.setItem('gga_coupon', JSON.stringify(appliedCoupon));
    } else {
      localStorage.removeItem('gga_coupon');
    }
  }, [appliedCoupon]);
  ...
  useEffect(() => {
    const syncRouteFromLocation = () => {
      ...
      else if (activeRoute.startsWith('/collection') || hash.startsWith('#collection')) {
        let routeContent = activeRoute.startsWith('/collection') ? activeRoute.replace('/collection', '') : hash.replace('#collection', '');
