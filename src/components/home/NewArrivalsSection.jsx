...
export default function NewArrivalsSection({ onSelectProduct, onAddToCart }) {
  const [arrivals, setArrivals] = useState([]);

  useEffect(() => {
    fetchNewArrivals().then(res => {
      if (res && Array.isArray(res) && res.length > 0) {
        // Show top 4 new arrival items on home section
        setArrivals(res.slice(0, 4));
      }
    });
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="font-serif italic text-xs sm:text-sm text-stone-500">
            Handcrafted Heritage Fresh From Jaipur Artisans
          </p>
        </div>

        {/* 4 Clean Arched Cards (2 per row on mobile, 4 per row on desktop) */}
            ...
            return (
              ...
              >
                ...
                <div className="relative w-full aspect-4/5 overflow-hidden bg-stone-100">
                  ...
                  />
                  <span className="absolute top-3 left-3 bg-amber-900 text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider shadow-sm">
                    NEW
                  </span>
                </div>
                ...
                <div className="p-3 sm:p-4 w-full flex-1 flex flex-col justify-between items-center text-center space-y-3">
                  ...
                  {/* Rectangular "Add to Cart" Button */}
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
